...
interface STIXImportModalProps {
  open: boolean;
  onClose: () => void;
...
export function STIXImportModal({
  open,
  onClose,
  onCreate,
  existingIOCs,
  ...
  const handleImport = async () => {
    ...
    setResults({ created, skipped, failed, relationshipsFound: relationshipCount });
    setStep('results');
    setImporting(false);
    if (created > 0) {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-200">
            {step === 'upload' && 'Import STIX 2.1 Bundle'}
        ...
        {step === 'upload' && (
          <div className="space-y-3">
            <p className="text-xs text-gray-400">
              Select a STIX 2.1 bundle JSON file to import indicators and vulnerabilities as IOCs.
            </p>
            ...
            <label className="flex items-center justify-center gap-2 px-4 py-6 rounded-lg border-2 border-dashed border-gray-700 hover:border-accent/50 text-gray-400 hover:text-accent cursor-pointer transition-colors">
              <Upload size={18} />
              <span className="text-sm">Choose STIX JSON file</span>
              <input
                ref={fileInputRef}
                type="file"
                accept=".json"
        ...
        {step === 'preview' && (
          <div className="space-y-3">
            <div className="space-y-1 text-xs text-gray-400">
              ...
              {relationshipCount > 0 && (
                <div>{relationshipCount} relationship{relationshipCount !== 1 ? 's' : ''} found</div>
              )}
        ...
        {step === 'results' && results && (
          <div className="space-y-4">
            <div className="space-y-2 text-sm">
              ...
              {results.relationshipsFound > 0 && (
                <div className="flex items-center gap-2 text-blue-400">
                  <span className="w-2 h-2 rounded-full bg-blue-400" />
                  {results.relationshipsFound} relationship{results.relationshipsFound !== 1 ? 's' : ''} detected
                </div>
              )}
            </div>
